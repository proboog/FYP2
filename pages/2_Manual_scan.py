...
def determine_best_min_support(Transaction):
    item_counts = {}
    ...
    item_counts = sorted(item_counts.values(), reverse=True)
    ...
    for count in item_counts:
        ...
        if cumulative_count / total_items >= top_n_percent:
            best_min_support = count / len(Transaction)
            break
    
    return best_min_support
...
with st.sidebar:
     st.header('Inputs')
     uploaded_csv = st.file_uploader('Upload CSV file', type=['csv'], accept_multiple_files=False, key=None, help='The file must be in CSV format and only one column', 
                                   on_change=None, args=None, kwargs=None, disabled=False, label_visibility="visible")
     st.header('Frequent itemset input')
     min_support = st.slider(label='Minimum support value' , value=0.1, min_value=0.01, step=0.01,max_value=1.0)
     st.header('Association rule input')
     min_conf = st.slider(label='Minimum confidence value' , value=0.1, min_value=0.01, step=0.01,max_value=1.0)
...
if uploaded_csv is not None:
    with main_container:
        ...
        if itemset_size == 1 and len(L[itemset_size]) == 0:
            st.warning(f"No frequent itemsets found. Try lowering the minimum support value")
        else:
